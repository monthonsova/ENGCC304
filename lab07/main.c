...
int main(void) {
    ...
    srand(time(NULL));  /* สุ่มเปลี่ยนไปตามเวลา */

    while (1) {
        printf("Do you want to play game (1=play,-1=exit) :\n");

        if (scanf("%d", &cmd) != 1) {
            scanf("%*s");  /* ทิ้งโทเคนที่ไม่ใช่ตัวเลข เช่น hi */
        ...
        /* เริ่มหนึ่งเกม */
        {
            ...
            while (1) {
                ...
                if (scanf("%d", &guess) != 1) {
                    scanf("%*s");  /* ทิ้งอินพุตที่ไม่ใช่ตัวเลข */
                    printf("\n");
                    continue;
                }
                ...
                if (guess == target) {
                    ...
                    break;  /* จบรอบนี้ กลับไปเมนู 1/-1 */
                }

                /* เดาผิด: หักคะแนนและบอกใบ้ */
                score -= 10;
